...
private:
  ...
  bool require_nonzero_generation_ = true;
  ...
  void readParameters()
  {
    ...
    private_nh_.param("require_nonzero_generation",
                      require_nonzero_generation_,
                      require_nonzero_generation_);
  ...
  bool validatePending(std::string &reason) const
  {
    ...
    const std::uint64_t generation = goal.header.seq;
    if (require_nonzero_generation_ && generation == 0)
    {
      reason = "REJECT_GENERATION";
      return false;
    }
    if (have_accepted_goal_ && generation <= accepted_generation_)
    {
  ...
  void tryForward()
  {
    ...
    accepted_generation_ = pending_goal_.header.seq;
